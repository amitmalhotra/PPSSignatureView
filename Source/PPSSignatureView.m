...
- (void)addTriangleStripPointsForPrevious:(PPSSignaturePoint)previous next:(PPSSignaturePoint)next
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        PPSSignaturePoint stripPoint = {{p1.x + difX, p1.y + difY, 0.0}, StrokeColor};
...
- (void)notifyOnFirstStroke
{
    if (dotsLength == 0 && length == 0 && self.signatureDelegate && [self.signatureDelegate respondsToSelector:@selector(onStrokeBegin:)])
    {
        [self.signatureDelegate performSelector:@selector(onStrokeBegin:) withObject:self];
    }
}

- (void)notifyOnErase
{
    if (self.signatureDelegate && [self.signatureDelegate respondsToSelector:@selector(onErase:)])
    {
        [self.signatureDelegate performSelector:@selector(onErase:) withObject:self];
    }
}
